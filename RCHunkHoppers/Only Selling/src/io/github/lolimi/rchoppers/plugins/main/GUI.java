...
public class GUI {
	...
	public static Inventory getSettings(boolean friend, ChunkHopper ch) {
		...
		ItemStack sellFilter = new ItemBuilder(Material.DIAMOND)
				.setName(conf.getString("GUI.Settings.Items.SellingFilter.Name",
						"§aClick here to set your selling filter"))
				.setLore(String.join(",",
						((List<String>) conf.getList("GUI.Settings.Items.SellingFilter.Lore",
								Arrays.asList("§3You can set a blacklist or a whitelist"))))
						.split(","))
	...
	public static Inventory getFilter(ChunkHopper h, boolean selling) {
		...
		ItemStack reset;
		if (selling) {
			inv = Bukkit.createInventory(null, 9 * 6, conf.getString("GUI.SellingFilter.Name", "§3Your Chunk Hopper selling filter:"));
			...
			add  = new ItemBuilder(Material.EMERALD_BLOCK)
					.setName(conf.getString("GUI.SellingFilter.Items.Explain.Emerald.Name", "§2To add an item, click it in your inventory"))
					.setLore(String.join(",",
							((List<String>) conf.getList("GUI.SellingFilter.Items.Explain.Emerald.Lore", Arrays.asList(""))))
							.split(","))
			...
			remove = new ItemBuilder(Material.REDSTONE_BLOCK)
					...
					.setLore(String.join(",",
							((List<String>) conf.getList("GUI.SellingFilter.Items.Explain.Redstone.Lore", Arrays.asList(""))))
							.split(","))
					.setLocalName("§1RCH.ChunkHopper").build();
			
			reset = new ItemBuilder(Material.BARRIER)
					.setName(conf.getString("GUI.SellingFilter.Items.Reset.Name", "§cTo reset your filter to default click here"))
					.setLore(String
							.join(",", ((List<String>) conf.getList("GUI.SellingFilter.Items.Reset.Lore", Arrays.asList(""))))
							.split(","))
