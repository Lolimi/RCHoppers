...
public class ChunkHopper extends RCHopper {
	...
	public ChunkHopper(@Nonnull Location loc, Integer tier, UUID placer) {
		location = loc;
		if(placer == null) {
	...
	public void saveOfflineSold() {
		for(OfflinePlayer p : offlineSold.keySet()) {
			Main.getOfflineSold().set(p.getUniqueId().toString(), offlineSold.get(p));
		}
	}
	...
	public static ItemStack getUpgradeItem(int amount, int tier) {
		...
		if(tier == 2) {
			m.setLore((List<String>) Main.getPlugin().getConfig().getList("UpgradeItem.Lore.1", Arrays.asList("§6Tier 1 §a➜ §6Tier 2")));
		}else {
			m.setLore((List<String>) Main.getPlugin().getConfig().getList("UpgradeItem.Lore.2", Arrays.asList("§6Tier 2 §a➜ §6Tier 3")));
		}
