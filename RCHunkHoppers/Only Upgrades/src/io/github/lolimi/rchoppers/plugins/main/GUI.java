...
public class GUI {
	...
	public static Inventory getSettings(boolean friend, int tier) {
		...
		if(tier == 2) {
			...
			ItemStack upgrade = new ItemBuilder(Material.GOLD_INGOT)
					.setName(conf.getString("GUI.Settings.Items.Upgrade.Name", "§aClick here to upgrade this Chunk Hopper to level 2"))
					.setLore(String.join(",",
							((List<String>) conf.getList("GUI.Settings.Items.Upgrade.Lore",
									Arrays.asList(""))))
							.split(","))
