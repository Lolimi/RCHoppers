...
import io.github.lolimi.rchoppers.plugins.commands.ChCommandTab;
import io.github.lolimi.rchoppers.plugins.commands.GetChunkHopperCommand;
import io.github.lolimi.rchoppers.plugins.commands.GetUpgradeCommand;
...
public class Main extends JavaPlugin {
	...
	public void onEnable() {
		...
		getCommand("gch").setTabCompleter(new ChCommandTab());
		getCommand("gu").setExecutor(new GetUpgradeCommand());
	...
	private void registerChunkHoppers() {
		...
		for(int i = 0; i < files.length; i++) {
			...
			hoppers[i] = new ChunkHopper(loc, null);
		}
